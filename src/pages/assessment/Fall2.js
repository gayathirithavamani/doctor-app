...
function Fall2() {
  ...
  const tableDataClick = (clickValue) => {
    ...
    if (clickValue == "data") {
      ...
      const value = result.filter((i) => i.fall_RISK_2 === clickValue);
  ...
  useEffect(() => {
    fetch("http://localhost:9090/findall ")
      ...
      .then((data) => {
        console.log(data);
        setResult(data);
        const yesValue = data.filter((i) => i.fall_RISK_2 === "Yes");
        const noValue = data.filter((i) => i.fall_RISK_2 === "No");
        const blankValue = data.filter((i) => i.fall_RISK_2 === null);
