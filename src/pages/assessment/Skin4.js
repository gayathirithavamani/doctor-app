...
function Skin5() {
  ...
  const tableDataClick = (clickValue) => {
    ...
    if (clickValue == "data") {
      ...
      const value = result.filter((i) => i.skin_RISK_5 === clickValue);
  ...
  useEffect(() => {
    fetch("http://localhost:9090/findall ")
      ...
      .then((data) => {
        console.log(data);
        setResult(data);
        const yesValue = data.filter((i) => i.skin_RISK_5 === "Yes");
        const noValue = data.filter((i) => i.skin_RISK_5 === "No");

        const blankValue = data.filter((i) => i.skin_RISK_5 === null);
