...
function Cat1() {
  ...
  const tableDataClick = (clickValue) => {
    ...
    if (clickValue == "data") {
      ...
      const value = result.filter((i) => i.cat1 === clickValue);
  ...
  useEffect(() => {
    fetch("http://localhost:9090/findall ")
      ...
      .then((data) => {
        console.log(data);
        setResult(data);
        const yesValue = data.filter((i) => i.cat1 === "Yes");
        const noValue = data.filter((i) => i.cat1 === "No");
              ...
              <tbody>
                {tableResult.map((item) => (
                  <tr key={item.id}>
                    ...
                    {yesValueShow ? (
                      <td>
                        {item.cat1 === "Yes" ? <CheckIcon /> : <CloseIcon />}
                      </td>
                    ) : null}
                    {noValueShow ? (
                      <td>
                        {item.cat1 === "No" ? <CheckIcon /> : <CloseIcon />}
                      </td>
                    ) : null}
                  </tr>
                ))}
